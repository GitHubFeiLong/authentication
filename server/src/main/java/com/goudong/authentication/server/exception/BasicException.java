...
import com.goudong.authentication.server.lang.AbstractDataMap;
import com.goudong.authentication.common.util.ArrayUtil;
...
import static com.goudong.authentication.server.exception.ServerExceptionEnum.*;
import static com.goudong.authentication.server.exception.ClientExceptionEnum.*;
...
public class BasicException extends RuntimeException implements BasicExceptionInterface {
    ...
    private static final long serialVersionUID = -1455037047375856238L;
    ...
    public static BasicException generateByServer(Throwable throwable) {
        // 默认500异常
        BasicException basicException = ServerException.server(SERVER_ERROR);

        // 空指针
        if (throwable instanceof NullPointerException) {
            return ServerException.server(SERVER_ERROR, "空指针异常", "NullPointerException null");
        }
        ...
        if (throwable instanceof RuntimeException) {
            return ServerException.server(SERVER_ERROR, throwable.getMessage(), "RuntimeException " + throwable.getMessage());
        }
    ...
    public static BasicException server() {
        return new ServerException(SERVER_ERROR);
    }
    ...
    public static BasicException server(Throwable e) {
        return new ServerException(SERVER_ERROR, e);
    }
    ...
    public static BasicException server(String serverMessage) {
        return new ServerException(SERVER_ERROR, serverMessage);
    }
    ...
    public static BasicException server(String clientMessage, String serverMessage) {
        return new ServerException(SERVER_ERROR, clientMessage, serverMessage);
    }
    ...
    public static BasicException server(String serverMessage, Throwable e) {
        return new ServerException(SERVER_ERROR, serverMessage, e);
    }
